...
void ghost_exchange(double* u, int n, int rank, int size)
{
    ...
    int ind;
    MPI_Status status;
    /* exchange to the right for processor except the last one */
    if (rank < size-1){
        /* send second last local element to the next processor */
        MPI_Send(&u[n-1], 1, MPI_DOUBLE, rank+1, 1, MPI_COMM_WORLD);
        /* receive from the next processor as the last local element */
        MPI_Recv(&u[n], 1, MPI_DOUBLE, rank+1, 1, MPI_COMM_WORLD, &status);
    }

    /* exchange to the left for processor except the first one */
    if (rank > 0){
        /* send second local element to the prevous processor */
        MPI_Send(&u[1], 1, MPI_DOUBLE, rank-1, 0, MPI_COMM_WORLD);
        /* receive from the prevous processor as the first local element*/
